    ...
    <aside className="w-[280px] bg-sidebar-bg border-r border-border flex flex-col h-screen p-4 pt-6 min-w-[280px]">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1">
        ...
        {documents.map((doc) => (
          <div
            key={doc.id}
            className={`relative w-full text-left p-3 rounded-[9px] border transition-all group ${
              selectedId === doc.id
                ? "border-accent bg-gradient-to-br from-accent/15 to-accent/5"
                : "border-border bg-panel/65 hover:border-[#3a4760] hover:bg-[#121b2b]"
            }`}
          >
            {/* Clickable area for selecting the document */}
            <button onClick={() => onSelect(doc)} className="w-full text-left">
            ...
            <button
              onClick={(e) => {
                ...
                if (
                  window.confirm(
                    `Delete "${doc.title}" and all its chat history?`,
                  )
                ) {
                  onDelete(doc.id);
                }
              }}
              className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 text-text-muted hover:text-danger transition p-1 rounded hover:bg-danger/10"
